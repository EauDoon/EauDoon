...
export function renderSelection(catalog, query, format) {
  ...
  if (format === 'ndjson') {
    const { projects, ...metadata } = report;
    return [{ kind: 'selection', ...metadata }, ...projects.map(project => ({ kind: 'project', project }))].map(row => JSON.stringify(row)).join('\n') + '\n';
  }
  if (format === 'csv') {
  ...
  throw new Error('Export format must be json, csv, markdown or ndjson');
}
