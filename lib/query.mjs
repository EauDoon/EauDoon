...
export function normalizeQuery(input, catalog) {
  ...
  const text = Object.hasOwn(input, 'text') ? input.text : '';
  ...
  const supplied = Object.hasOwn(input, 'filters') ? input.filters : {};
  ...
  const exclude = Object.hasOwn(input, 'exclude') ? input.exclude : [];
...
export function facets(catalog, query) {
  const choices = { runtime: vocab.runtimes, category: vocab.category, privacy: vocab.privacy, fork: ['yes', 'no'], task: [...new Set(catalog.projects.flatMap(p => p.tasks))].sort() };
  return {
    assessedOn: catalog.assessedOn, query, total: runQuery(catalog, query).length,
    note: 'Each count replaces that one filter with the named value, preserving all other constraints and exclusions.',
    facets: Object.fromEntries(Object.entries(choices).map(([key, values]) => [key, values.map(value => ({ value, selected: query.filters[key]?.includes(value) ?? false, count: runQuery(catalog, { ...query, filters: { ...query.filters, [key]: [value] } }).length }))])),
  };
}
