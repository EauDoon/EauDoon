...
export function queryImpact(before, after, input) {
  ...
  const previousIds = new Set(left.map(p => p.id.toLowerCase())); const nextIds = new Set(right.map(p => p.id.toLowerCase()));
  ...
  return {
    ...
    entered: right.filter(p => !previousIds.has(p.id.toLowerCase())).map(p => p.id),
    left: left.filter(p => !nextIds.has(p.id.toLowerCase())).map(p => p.id),
    retained: right.filter(p => previousIds.has(p.id.toLowerCase())).map(p => p.id),
    changedRetained: changes.changed.filter(p => previousIds.has(p.id.toLowerCase()) && nextIds.has(p.id.toLowerCase())),
