...
import { batchQueries, coverageMatrix, compareQueries, freshness } from './review.mjs';
...
export const workflows = {
  freshness(catalog, args) {
    requireArgs(args, 2, 'freshness AS-OF-DATE MAXIMUM-DAYS');
    if (!/^(0|[1-9][0-9]{0,4})$/.test(args[1])) throw new Error('Maximum age must be an integer from 0 to 36500 days');
    return freshness(catalog, args[0], Number(args[1]));
  },
  'compare-queries'(catalog, args) {
    requireArgs(args, 2, 'compare-queries BEFORE-QUERY.json AFTER-QUERY.json');
    return compareQueries(catalog, readJson(args[0]), readJson(args[1]));
  },
  coverage(catalog, args) {
    requireArgs(args, 2, 'coverage QUERY.json TASKS.json');
    return coverageMatrix(catalog, normalizeQuery(readJson(args[0]), catalog), readJson(args[1]));
  },
  batch(catalog, args) {
    requireArgs(args, 1, 'batch BRIEFS.json');
    return batchQueries(catalog, readJson(args[0]));
  },
