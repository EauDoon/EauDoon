...
export function selectProject(projects, id) {
  ...
  if (!project) throw new Error('Unknown project. Use list or search to find an exact id.');
  return project;
}
...
export function parseOptions(args, projects) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (Object.hasOwn(options, key)) throw new Error('Repeated option');
    if (key === 'json') { options.json = true; continue; }
    if (!Object.hasOwn(allowed, key)) throw new Error('Unknown option');
    const value = args[++i];
